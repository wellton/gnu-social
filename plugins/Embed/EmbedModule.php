...
/**
 * Base class for the Embed module that does most of the heavy lifting to get
 * and display representations for remote content.
 *
 * @copyright 2019 Free Software Foundation, Inc http://www.fsf.org
 * @license   https://www.gnu.org/licenses/agpl.html GNU AGPL v3 or later
 */
class EmbedModule extends Module
{
    ...
    protected function validateAndWriteImage(&$imgData, $url = null, $headers = null, $file_id = 0) : array
    {
        ...
        try {
            ...
            common_log(LOG_ERR, "Went to write a thumbnail to disk in EmbedModule::storeRemoteThumbnail " .
    ...
    protected function storeRemoteFileThumbnail(File_thumbnail $thumbnail)
    {
        ...
        try {
            ...
            common_log(LOG_ERR, "Went to write a thumbnail entry to the database in " .
                       "EmbedModule::storeRemoteThumbnail but encountered error: ".$err);
            return $err;
        }
        return true;
    }
    ...
    public function onModuleVersion(array &$versions)
    {
        $versions[] = ['name' => 'Embed',
                       ...
                       _m('Module for using and representing oEmbed, OpenGraph and other data.')];
        return true;
    }
}
