...
class Activitypub_follow
{
    ...
    public static function follow(Profile $actor_profile, string $object, string $id)
    {
        ...
        if (!Subscription::exists($actor_profile, $object_profile)) {
            ...
            common_debug('ActivityPubPlugin: Accepted Follow request from '.ActivityPubModule::actor_uri($actor_profile).' to '.$object);
        } else {
            common_debug('ActivityPubPlugin: Received a repeated Follow request from '.ActivityPubModule::actor_uri($actor_profile).' to '.$object);
        }

        // Notify remote instance that we have accepted their request
        common_debug('ActivityPubPlugin: Notifying remote instance that we have accepted their Follow request request from '.ActivityPubModule::actor_uri($actor_profile).' to '.$object);
